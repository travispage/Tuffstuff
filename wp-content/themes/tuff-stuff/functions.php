...
function mttr_scripts() {
	...
	// Base Stylesheet
	wp_enqueue_style( 'mttr-style', get_template_directory_uri() . '/assets/css/main.css?v=161005' );

	// SVG Injector
	wp_enqueue_script( 'svg-injector', get_template_directory_uri() . '/assets/js/vendor/svg-injector.min.js', array(), '20150710', true );

	// Base Scripts
	wp_enqueue_script( 'mttr-scripts', get_template_directory_uri() . '/assets/js/scripts.min.js?v=201016', array('jquery','svg-injector'), '20150623', true );
...
function mttr_get_types( $types, $parent_term ) {
	...
	if ( $types ) {
		...
		foreach( $types as $type ) {
			...
			$term = get_term( $type, 'pa_part-types' );
			
			$output .= '<li class="menu-item  type-menu-item  g-one-fifth">';

				$output .= '<a href="' . get_term_link( $parent_term ) . '?fwp_type=' . $term->slug . '">';

					$image = get_field( 'mttr_pa_types_image', $term );
					...
					if ( $image ) {
						...
						$output .= '<img src="' . get_stylesheet_directory_uri() . '/assets/img/placeholder-white.png">';
...
function mttr_get_brands( $brands, $parent_term ) {
	...
	if ( $brands ) {
		...
		foreach( $brands as $brand ) {
			...
			$term = get_term( $brand, 'product_brand' );
			
			$output .= '<li class="menu-item  brand-menu-item">';

				$output .= '<a href="' . get_term_link( $parent_term ) . '?fwp_brands=' . $term->slug . '">';
...
add_filter( 'woocommerce_checkout_fields' , 'mttr_override_checkout_fields' );
 
function mttr_override_checkout_fields( $fields ) {
...
class mttr_sublevel_walker extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( $item->type == 'custom' && ( $item->title == 'Brands' || $item->title == 'Types' ) &&  $depth == 1 ) {
        ...
        if ( $item->type == 'custom' && $item->title == 'Brands'  &&  $depth == 1 ) {
	        ...
	        if ( $parent_id && $parent_term ) {

	        	$brands = get_field( 'mttr_menu_brands', $parent_term );

	        	if ( $brands ) {

	        		$item_output .= '<ul class="sub-menu  u-text-columns  u-text-columns--thirds">';

           				$item_output .= mttr_get_brands( $brands, $parent_term );
        ...
        if ( $item->type == 'custom' && $item->title == 'Types'  &&  $depth == 1 ) {

            $menu_item_parent = $item->menu_item_parent;
        	$parent_id = get_post_meta( $item->menu_item_parent, '_menu_item_object_id', true );

        	$parent_term = get_term( $parent_id, 'pa_part-types' );

	        if ( $parent_id && $parent_term ) {

	        	$types = get_field( 'mttr_menu_part_types', $parent_term );

	        	if ( $types ) {

	        		$item_output .= '<ul class="sub-menu  sub-menu--types">';

           				$item_output .= mttr_get_types( $types, $parent_term );
    ...
    function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( $depth == 0 ) {
			...
			$output .= '<div class="mega-menu  band  band--secondary  u-text--small">';

				$output .= '<div class="band__body">';
...
// 	// add_filter( 'wp_nav_menu_items', 'mttr_custom_menu_items', 10, 2 );
...
// 			$output .= '<div class="mega-menu  band  band--large  band--secondary  u-hard--top">';
 ...
 *	This function is for outputting the 'next' post so we can hook it up with Selenium for saving posts
 */
function mttr_acf_test_comment_override( $field ) {

	global $post;
    $post->ID;

    $next_post = ( intval( $post->ID ) + 1 );

    var_dump($next_post);
     ?>
 	<a id="mttr_next_product" href="<?php echo 'http://tuff.dev.mattersolutions.com/wp-admin/post.php?post=' . $next_post; ?>&action=edit"><?php echo get_the_title( $next_post->ID ); ?></a>
	<?php
    return $field;
		 
}

add_filter('acf/load_field/key=field_57d9e8211029f', 'mttr_acf_test_comment_override');