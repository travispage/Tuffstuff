...
//		}
		$settings = get_option('w3exabe_settings');
			...
			{
				if(isset($settings['dontshowhint1']))
				{
					if($settings['dontshowhint1'] == 1)
					{
						$dontshowhint = true;
					}
				}
		...
		if(is_array($sel_fields) && !empty($sel_fields))
		{
		    ...
		    if(!$dontshowhint)
		    {
				if(count($sel_fields) !== 11 && count($sel_fields) !== 18)
			    {
					$dontshowhint = true;
				}else
				{
					if(count($sel_fields) === 11)
					{
						if(!isset($sel_fields["_visibility"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_stock_status"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_stock"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_sale_price"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_regular_price"]))
							$dontshowhint = true;
						if(!isset($sel_fields["post_status"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_sku"]))
							$dontshowhint = true;
						if(!isset($sel_fields["product_cat"]))
							$dontshowhint = true;
						if(!isset($sel_fields["post_title"]))
							$dontshowhint = true;
					}
					if(count($sel_fields) === 18)
					{
						if(!isset($sel_fields["_visibility"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_stock_status"]))
						...
						if(!isset($sel_fields["post_title"]))
							$dontshowhint = true;
						//new
						if(!isset($sel_fields["_product_permalink"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_tax_status"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_featured"]))
							$dontshowhint = true;
						if(!isset($sel_fields["post_content"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_product_image_gallery"]))
							$dontshowhint = true;
						if(!isset($sel_fields["_thumbnail_id"]))
							$dontshowhint = true;
					}
...
//				}
		...
		<br/>
			...
			<?php 
				...
				if(!$dontshowhint)
				{
					echo '<div id="divhidehint1" style="background-color:#FFFFE0;border: 1px solid #E6DB55;padding:5px;height: 29px; line-height: 29px;">'.__( '<b>Hint:</b> To show different woocommerce fields use the "Show/Hide Fields" button at the bottom.', 'woocommerce-advbulkedit').'   <input id="hidehint1" class="button" type="button" value="'.__( 'Hide', 'woocommerce-advbulkedit').'" /></div>';
				}
			?>		
